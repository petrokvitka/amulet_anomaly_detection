...
### Testing
To test the application, the [wav file](https://github.com/petrokvitka/amulet_anomaly_detection/blob/master/test_1200_200.wav) is provided, which was recorded with the same motor settings as the training data for the model we are testing. The anomaly we want to catch in this case is a motor stopping at several points. 

There are two ways to test the AMULET application. A GUI is currently still in development.
...
### Looking forward
The AMULET is a FLASK application, which is a common way to use python applications in web. This will allow us to create nice and user-friendly GUI with HTML and CSS.
